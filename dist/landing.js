...
const DESCRIPTION = "Three x402 micropayment APIs for AI agents: Algorand wallet risk scoring, LLM text " +
...
exports.TOOLS = [
    {
        ...
        blurb: "Explainable 0-100 risk score for any Algorand address, computed from real on-chain " +
            "indexer data. No LLM — deterministic and auditable.",
        input: "An Algorand address (58 characters) in the URL path.",
        output: "{ address, riskScore (0-100), riskLevel (low|medium|high), signals: { accountAgeDays, " +
    ...
    },
    {
        ...
        blurb: "Send a natural-language prompt, receive generated text. Powered by Claude Haiku 4.5.",
    ...
    },
    {
        ...
        blurb: "Condense up to 50,000 characters into a concise summary, with optional length and style " +
            "control.",
        input: '{ "text": string (max 50000 chars), "maxWords"?: number, ' +
            '"style"?: "concise" | "bullets" | "detailed" }',
        output: '{ "summary": string, "truncated": boolean }',
    },
];
function renderLandingPage() {
    const cards = exports.TOOLS.map((t) => `      <article class="tool">
        <div class="tool-head">
...
make_http_request_with_x402 "https://YOUR_DOMAIN/api/wallet-risk/&lt;ALGORAND_ADDRESS&gt;"</code></pre>
    ...
    <div class="meta">
      <p>
        Settlement: USDC (ASA <code>${escapeHtml(config_1.USDC_ASA_ID)}</code>) on Algorand mainnet via the
        GoPlausible facilitator. All endpoints pay to a single address:
        <code>${escapeHtml(config_1.PAY_TO)}</code>.
      </p>
...
function renderLlmsTxt(baseUrl) {
    const tools = exports.TOOLS.map((t) => `### ${t.name} — ${t.price} per call
...
- Asset: USDC (ASA ${config_1.USDC_ASA_ID})
- Facilitator: https://facilitator.goplausible.xyz
- Pay-to address (shared by all endpoints): ${config_1.PAY_TO}

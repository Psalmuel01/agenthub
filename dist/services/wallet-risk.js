...
exports.InvalidAddressError = exports.WalletRiskError = void 0;
...
class WalletRiskError extends Error {
    constructor(message) {
        super(message);
        this.name = "WalletRiskError";
    }
}
exports.WalletRiskError = WalletRiskError;
...
async function indexerGet(path) {
    let resp;
    try {
        resp = await fetch(`${config_1.INDEXER_URL}${path}`);
    }
    catch (err) {
...
async function scoreWallet(address) {
    if (!(0, avm_1.isValidAlgorandAddress)(address)) {
    ...
    const txResp = await indexerGet(`/v2/accounts/${address}/transactions?limit=${TX_LIMIT}`);
    ...
    const usdcOptedIn = Array.isArray(account?.assets)
        ? account.assets.some((a) => String(a["asset-id"]) === String(config_1.USDC_ASA_ID))
        : false;
    ...
    const accountAgeDays = createdRound > 0 && currentRound > createdRound
        ? Math.round(((currentRound - createdRound) * 2.8) / 86400)
        : null;
    ...
    if (accountAgeDays === null)
        score += 30; // no visible history at all
